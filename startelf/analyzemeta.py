...
def analyze_ss(name, xli, xd):
    if len(xli) == 0:
        return
    print(name)
    for datadict in xli:
        sslist = datadict['info'].split('\n')
        for i, ssinfo in enumerate(sslist):
            # if re.search('\s+10\.\d+\.\d+\.\d+', ssinfo):
            if re.search('iperf', ssinfo):
                print(datadict['timestamp'], ssinfo, sslist[i+1])
